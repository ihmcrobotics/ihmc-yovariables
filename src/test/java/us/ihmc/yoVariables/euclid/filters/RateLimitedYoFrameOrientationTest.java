...
public class RateLimitedYoFrameOrientationTest
{
   ...
   public void testConvergenceWithConstantInput() throws Exception
   {
      ...
      FiniteDifferenceAngularVelocityYoFrameVector3D angularVelocity = new FiniteDifferenceAngularVelocityYoFrameVector3D("rate", ReferenceFrame.getWorldFrame(),
                                                                                                                          dt, registry);
      ...
      for (int i = 0; i < 1000; i++)
      {
         ...
         double distanceToGoal = Math.abs(EuclidCoreTools.trimAngleMinusPiToPi(new Quaternion(rateLimitedOrientation).distance(goalQuaternion)));
         if (distanceToGoal / dt < maxRate.doubleValue())
         { // Should converge in one step
            rateLimitedOrientation.update(goalQuaternion);
            EuclidCoreTestTools.assertOrientation3DGeometricallyEquals(goalQuaternion, new Quaternion(rateLimitedOrientation), EPSILON);
         }
         else
         {
            ...
            {
               rateLimitedOrientation.update(goalQuaternion);
               double distance = Math.abs(EuclidCoreTools.trimAngleMinusPiToPi(new Quaternion(rateLimitedOrientation).distance(goalQuaternion)));
