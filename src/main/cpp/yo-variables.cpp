...
YoVariable::set_registry(std::shared_ptr<YoRegistry>& registry)
{
    ...
    if (registry != nullptr)
    {
        ...
        registry->add_variable(this);
    }
    registry_ = registry;
    reset_full_name();
}

std::shared_ptr<YoRegistry>& YoVariable::get_registry() const 
{ 
    return registry_; 
}
